...
int main(int argc, char *argv[])
{
  ...
  int recvAdrSz;
  long unsigned int flagInfo = 0, mode = 1;
...
void CALLBACK WriteCompRoutine(DWORD dwError, 
                      DWORD szSendBytes, LPWSAOVERLAPPED lpOverlapped, DWORD flags)
{
  LPPER_IO_DATA hbInfo = (LPPER_IO_DATA)(lpOverlapped->hEvent);
  SOCKET hSock = hbInfo->hClntSock;
  LPWSABUF bufInfo = &(hbInfo->wsaBuf);
  DWORD recvBytes;
  long unsigned int flagInfo = 0;
